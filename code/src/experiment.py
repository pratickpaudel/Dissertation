...
def config_id(dataset: str, method: str, classifier: str, ratio: float) -> str:
    """Stable identifier used for result rows and prediction filenames."""
    return f"{dataset}__{method}__{classifier}__r{int(round(ratio * 100))}"
...
def run_configuration(
    ...
    cid = config_id(dataset, method, classifier, minority_ratio)
    if save_predictions:
        np.savez_compressed(
...
def run_all(
    ...
    if verbose:
        print(f"Running {total} configurations at {minority_ratio:.0%} minority ratio\n")

    for i, (dataset, method, classifier) in enumerate(combinations, start=1):
        if verbose:
            print(f"[{i:>3}/{total}] {dataset:10s} {method:22s} {classifier:15s}", end=" ", flush=True)

        try:
            row = run_configuration(dataset, method, classifier, minority_ratio)
            rows.append(row)
            if verbose:
                print(
            ...
            rows.append(
                {
                    "config_id": config_id(dataset, method, classifier, minority_ratio),
                    "dataset": dataset,
                    "imbalance_method": method,
                    "classifier": classifier,
                    "minority_ratio": minority_ratio,
