"""
Feature extraction from raw URLs (supports the dashboard in Section 3.10).
...
This matters for validity: if the extraction here differs from the definitions
used to build the dataset, the model would receive inputs drawn from a different
distribution than it was trained on, and its predictions would be unreliable.
The definitions below were therefore recovered from the published data and are
checked against it by ``verify``, which reports the exact-match rate per feature.

Feature semantics recovered from the data:

* counts and ratios are computed over the **whole URL string**, including scheme
* ``dom`` is the registrable domain, that is the public suffix plus one label
* ``tld`` is the public suffix, which may span several labels (``gov.au``)
* ``subdom_cnt`` counts the labels preceding the registrable domain
* for an IP host there is no suffix, so ``tld`` is empty and ``subdom_cnt`` is 0
* ``entropy`` is the Shannon entropy of the URL string in bits
"""
